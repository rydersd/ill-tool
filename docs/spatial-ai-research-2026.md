...
Based on this research, we implemented Option A (Full 3D reconstruction pipeline) with the following toolchain:

1. **TRELLIS.2** for mesh reconstruction (integrated, awaiting model download for end-to-end test)
2. **Custom mesh face grouper** — clusters faces by normal direction, extracts boundaries, projects to 2D
3. **Path gradient approximation** — pure-Python DiffVG fallback using finite-difference gradients with OpenCV rasterization
4. **Self-correcting feedback loop** — projection errors stored as correction deltas, pre-applied to future runs

### Key Findings From Implementation

1. **The research was right about LLM coordinate precision.** Seven trace attempts scored 0-0.3/10. The constraint pipeline (LLM labels + solver computes) found 14K correct form edges but assembled them into a 0/10 trace. Assembly is spatial reasoning the LLM can't do.

2. **The "go through 3D" approach eliminates shadow-vs-form ambiguity by construction.** Meshes have geometry, not lighting artifacts. Face grouping by normals produces exactly the planes an illustrator would identify.

3. **Evaluator calibration matters more than pipeline sophistication.** Our initial evaluator (multi-factor subjective blend) scored 4.6/10 on work that was 0.3/10. Replacing it with Hausdorff-based pixel deviation gave honest scores. You can't improve what you can't measure.

4. **The feedback loop is novel and works on synthetic data.** Storing projection deltas and pre-applying them to future runs is the same pattern as DWPose correction learning, extended to face boundaries. Each run improves the next. Awaiting real-image validation.

5. **Pure-Python fallbacks are essential.** DiffVG requires C++ compilation that fails on macOS ARM. The finite-difference gradient approximation (OpenCV rasterization + numerical gradients) is 100x slower but always available. TRELLIS.2 may need GitHub clone instead of pip install.
...
- Pipeline performance on subjects with complex topology (overlapping parts, thin features)
